								...
								<div className="thermInfo" key={inf.id}>
									<div className="therm__characteristics">
										{inf.characteristics.map((char, index) => (
											<p className="therm__characteristics-item" key={index}>
												{char}
											</p>
										))}
									</div>
									...
									<div className="therm__buttons">
										<div className="therm__buttonsCalc">
											<button className="button__amMinus" onClick={() => setAmount(amount - 1)}>
												-
											</button>
											<input className="therm__am" type="text" value={amount} onChange={e => amHandler(e)} />
											<button className="button__amPlus" onClick={() => setAmount(amount + 1)}>
												+
											</button>
										</div>
									...
									<div className="therm__desc">
										{inf.desc.map((d, index) => (
											<p className="therm__desc-item" key={index}>
												{d}
											</p>
										))}
									</div>
							...
							{techSpecs.map(t => (
								<div className="techSpecs__content" key={t.id}>
									...
									<div className="techSpecs__text" key={t.id}>
										<span style={{ marginBottom: '14px' }} className="small__topTitle">
											{t.topTtl}
										</span>
										<h1 className="therm__title">{t.title}</h1>
										<h1 style={{ marginTop: '28px' }} className="therm__subTitle">
											{t.subtitle1}
										</h1>
										<span className="therm__spec" data-end={t.dimensions1}>
											Dimensions
										</span>
										<span className="therm__spec">Cooking temperature range</span>
										{t.temp.map((line, i) => (
											<p key={i} className="temperature__line">
												{line}
											</p>
										))}
										<span className="therm__spec" data-end={t.battery1}>
											Rechargeable internal battery
										</span>
										<span className="therm__spec" data-end={t.battery1}>
											Water-resistant probe thermometer{' '}
										</span>
										<span className="therm__spec" data-end={t.materials}>
											Materials
										</span>
										<span className="therm__spec" data-end={t.bluetooth}>
											Bluetooth® wireless
										</span>
										<h1 style={{ marginTop: '18px' }} className="therm__subTitle">
											{t.subtitle2}
										</h1>
										<span className="therm__spec" data-end={t.dimensions2}>
											Dimensions
										</span>
										<span className="therm__spec" data-end={t.range}>
											Range
										</span>
										<span className="therm__spec" data-end={t.range}>
											Batteries
										</span>
										<span className="therm__spec" data-end={t.magn}>
											Magnetic base
										</span>
