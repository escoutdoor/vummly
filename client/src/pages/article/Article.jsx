...
const Article = () => {
	...
	useEffect(() => {
		const fetch = async () => {
			await axios.get(`/support/articles/one/${article}`).then(res => {
						...
						<div className="article__info">
							<div className="articleLeft">
								...
								<ul className="articleList">
									{articleInfo.section?.[0].list.map((l, i) => (
										<Link
											onClick={() => {
												articleInfo.idPage !== l.link && setLoading(false)
											}}
											to={`/support/articles/${ctgr}/${l.link}`}
											key={i}>
											<li className={articleInfo.idPage === l.link ? 'articleList-item active' : 'articleList-item'}>{l.name}</li>
										</Link>
									))}
								</ul>
							</div>
							<div className="articleText">
								...
								<div className="article__update">
									<p className="article__updatetime" data-end={articleInfo.updatedAt != articleInfo.createdAt ? 'updated' : null}>
										{moment(articleInfo.updatedAt).fromNow()}
									</p>
								</div>
										...
										{item.text
											? item.text.map((line, i) => (
													<li className="article__textline" style={{ fontStyle: `${line.fontstyle ? line.fontstyle : 'normal'}` }} key={i}>
														...
														<Link className="article__mail" to={line.link || `mailto:${line.mail}`}>
															{line.link || (line.mail && line.link) || line.mail}
														</Link>
														{line.image &&
															line.image.map((image, index) => <img key={index} className="article__Image" src={`${PF}images/img/articles/${image.img}`}></img>)}
														{line.bigImage &&
															line.bigImage.map((image, index) => <img key={index} className="article__bigImage" src={`${PF}images/img/articles/${image.img}`} />)}
													</li>
											  ))
											: null}
										{item.points
											? item.points.map((p, i) => (
													<li className="article__point" style={{ listStyle: `${p.point || p.bold ? 'disc' : 'none'}` }} key={i}>
														...
														{p.link || p.mail ? (
															<Link className="article__mail" to={p.link || `mailto:${p.mail}`}>
																{p.point || p.mail}
															</Link>
														) : (
															p.point
														...
														{p.image
															? p.image.map((image, index) => <img key={image._id} className="article__Image" src={`${PF}images/img/articles/${image.img}`}></img>)
															: null}
													</li>
											  ))
											: null}
								...
								<div className="articleRecently">
									<div className="articleRecently__viewed">
										...
										<ul className="recommendationList">
											{recentlyViwed ? (
												recentlyViwed.map(
													(rec, index) =>
														rec.title !== articleInfo.title && (
															<Link
																style={{ width: 'fit-content' }}
																onClick={() => {
																	articleInfo.idPage !== rec.link && setLoading(false)
																}}
																to={`/support/articles/${allSections && allSections.find(item => item.list && item.list.some(l => l.link === rec.link))?.own}/${
																	rec.link
																}`}
																key={index}>
									...
									<div className="articleRecently__related">
										...
										<ul className="recommendationList">
											{articleInfo.section[0].list
												.reverse()
												.slice(articleInfo.section[0].list?.length >= 7 ? articleInfo.section[0].list?.length / 2 : 0)
												.map((listItem, i) => (
													<Link
														style={{ maxWidth: 'fit-content' }}
														onClick={() => {
															articleInfo.idPage !== listItem.link && setLoading(false)
														}}
														to={`/support/articles/${ctgr}/${listItem.link}`}
														key={i}>
