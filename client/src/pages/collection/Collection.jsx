...
const Collection = () => {
	...
	const handleChanges = async () => {
		if (collection.name !== name.trim() && name.length !== 0 && user._id) {
			await axios
				.put(`/collections/name/${collection._id}/${user._id}`, {
					name: name.trim(),
				})
				.then(c => {
					nav(`/profile/${userId}/collections/${c.data.name}`)
					setCollection(c.data)
				})
		} else if (collection.description !== description.trim() && user._id) {
			await axios
				.put(`/collections/description/${collection._id}/${user._id}`, {
				...
				<div className={s.main}>
					<div className="wrap1160">
						<div className={s.mainContent}>
							{recipesLastAdded.length !== 0 && collectionName !== 'all-vums' && (
								<div className={s.mainHeader}>
									<div className={s.sortMenu}>
										<div className={s.text} onClick={() => setActiveSortMenu(!activeSortMenu)}>
											<h1 style={{ color: '#bababa' }} className={s.sortTitle}>
												Sort By
											</h1>
							...
							<div className={s.recipes}>
								{sortSettings === 'last added'
									? recipesLastAdded.map(r => <RecipeItem key={r._id} recipe={r} rating={r.rating} />)
									: sortSettings === 'recipe name'
									? recipesByName.map(r => <RecipeItem key={r._id} recipe={r} rating={r.rating} />)
									: null}
