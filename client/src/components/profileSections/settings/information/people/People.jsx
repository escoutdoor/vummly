...
const People = ({ active, user, setPeople, people }) => {
	...
	const choosePeople = async (adults, kids) => {
		if (user._id) {
			await axios
				.put(`/settings/household/${user._id}`, {
					adults: adults,
					kids: kids,
				})
				.then(p => {
					...
					<div className={s.input}>
						<div className={s.input__status}>
							...
							<div className={s.arrows}>
								...
								<img
									className={s.icon}
									onClick={() => people.adults > 0 && choosePeople(people.adults - 1, people.kids)}
									src={`${PF}images/icons/profile/settings/down.svg`}
									alt="imgBubble"
								/>
							</div>
						</div>
					...
					<div className={s.input}>
						<div className={s.input__status}>
							...
							<div className={s.arrows}>
								...
								<img
									className={s.icon}
									onClick={() => people.kids > 0 && choosePeople(people.adults, people.kids - 1)}
									src={`${PF}images/icons/profile/settings/down.svg`}
									alt="imgBubble"
								/>
							</div>
