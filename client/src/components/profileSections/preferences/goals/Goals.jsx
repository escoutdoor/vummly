...
const Goals = ({user, active, setActive, goals, setGoals, loaded}) => {
    ...
    const deleteGoal = async (item) => {
        if(item && user._id) {
            await axios.put(`/preferences/deleteGoal/${user._id}`, {
                title: item
            }).then((d) => {
    ...
    const addGoal = async (item) => {
        if(user._id && item.trim().length !== 0) {
            await axios.put(`/preferences/addGoal/${user._id}`, {
                title: item,
            }).then((d) => [
                setGoals(d.data.goals)
            ])
        ...
        <div className={s.block}>
            ...
            <div className={s.main}>
                <div className={s.topbar}>
                    ...
                    {goals.map((d, index) => (
                        <div className={s.bubble} style={{width: '150px', height: '150px'}} key={index} onClick={() => {deleteGoal(d)}}>
                    ...
                    <div onClick={() => {setActive(active ? "" : 'goals')}} className={s.addButton}>
                ...
                <div className={active ? `${s.menu} ${s.active}` : s.menu}>
                    <div className={s.menu__content}>
                        {defaultGoals.length !== 0 ? defaultGoals.map((d, index) => (
                            <div className={s.bubble} key={index} onClick={() => {addGoal(d.title)}}>
