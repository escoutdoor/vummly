...
const EmailPage = ({setPage, mailError, mail, setMail}) => {
    ...
    const goNext = (event) => {
        event.key === 'Enter' && mail && !mailError && setPage('password')
    }

    return (
        <div className={s.second} onKeyDown={goNext}>
            <button className={s.back} onClick={() => setPage('start')}>back</button>
            ...
            <div className={s.mail}>
                ...
                <div className={mail && mailError ? `${s.alert} ${s.active}` : s.alert}><p className={mail && mailError ? `${s.alertText} ${s.active}` :  s.alertText}>{mailError}</p></div>
            </div>
            <button title='Next' className={s.nextButton} onClick={() => mail && !mailError && setPage('password')}>Next</button>
