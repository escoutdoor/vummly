			...
			<div className={styles.search__container}>
				<div className={styles.searchRecipes}>
					...
					<div
						...
						className={searchActive ? styles.searchRecipes__drop : `${styles.searchRecipes__drop} ${styles.hide}`}>
						<div
							onClick={e => e.stopPropagation()}
							className={searchActive ? styles.searchRecipes__dropContent : `${styles.searchRecipes__dropContent} ${styles.hide}`}>
							<div className={styles.drop__search}>
								...
								<input
									...
									type="search"
									className={styles.drop__searchInput}
								/>
							</div>
							<div className={styles.drop__recent}>
								...
								<div className={styles.recentSearches}>
									{searchHistory.map((s, index) => (
										<span key={index} onClick={() => navigate('/recipes', { state: { query: s } })} data-end={searchHistory[4] === s ? '' : ','}>
											{s}
										</span>
									))}
								</div>
						...
						feed.map(f => (
							<SwiperSlide className={styles.swiperSearch__item} key={f.id}>
								<div className={styles.searchWrapper}>
										...
										<ul className={styles.swiperList}>
											{f.points ? (
												f.points.map((point, index) =>
													point.label ? (
														<li className={styles.swiperList__item} key={point.id}>
															{point.label}
														</li>
													) : (
														<div key={point.id} className={styles.swiperList__itemImg}>
															{point.image.map(i => (
																<img key={i.id} src={`${PF}images/img/myFeed/${i.img}`}></img>
															))}
														</div>
													)
												)
									...
									{f.images &&
										f.images.map(i => (
											<div key={i.id} className={styles.swiperRight}>
