...
const RecipeIngredients = ({
	...
	const addToShoppingList = async ingredient => {
		try {
			if (user && recipe) {
				await axios.put(`/shopping/${user?._id}`, {
					name: ingredient.ingredient,
					measurement:
						ingredient.measurement === 'grams' ? 'kgs' : ingredient.measurement === 'ml' ? 'ltrs' : '',
					quantity:
						Number.isInteger(ingredient.quantity) && !ingredient.measurement ? ingredient.quantity : 1,
					recipeId: recipe._id,
				})
			} else {
				setActiveModal(true)
	...
	const addAllToShoppingList = async ingredients => {
		try {
			if (user && recipe) {
				await axios.put(`/shopping/addAll/${user?._id}`, {
		...
		<div className={s.ingredients}>
			<div className={s.header}>
				...
				<div className={s.header__right}>
					...
					<h1 className={s.servings}>
						{recipe.servings ? `${recipe.servings} servings` : <Skeleton width={80} height={12} />}
					</h1>
				</div>
			</div>
			...
			<div className={s.buttons}>
				...
				>
					...
					<img
						src={
							madeIt
								? `${PF}images/icons/recipes/check-circleActive.svg`
								: `${PF}images/icons/recipes/check-circle.svg`
						}
						alt="checkedRecipe"
					/>
				</div>
				<div className={s.manage}>
					<div className={s.mealPlanner} onClick={() => addMealPlanner()}>
						...
						<p className={s.mealPlanner__button}>
							{recipe.isAdded ? 'Remove from Meal Planner' : 'Add to Meal Planner'}
						</p>
					</div>
				</div>
			</div>
