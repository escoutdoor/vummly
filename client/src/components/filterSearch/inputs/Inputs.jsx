...
const Inputs = ({ ingredients, setWith, setWithout, visibility, toolTips, setToolTips, allowedRequest, setAllowedRequest, bannedRequest, setBannedRequest }) => {
	...
	useEffect(() => {
		setToolTips(
			ingredients.filter(ingredient => ingredient.toLowerCase().includes(searchValue.toLowerCase()) && !allowedRequest.find(a => a === ingredient) && !bannedRequest.find(a => a === ingredient))
		)
	...
	useEffect(() => {
		setToolTips(
			ingredients.filter(
				ingredient => ingredient.toLowerCase().includes(searchValueWithout.toLowerCase()) && !bannedRequest.find(a => a === ingredient) && !allowedRequest.find(a => a === ingredient)
			)
		)
			...
			<div className={s.filter__container}>
				<div className={s.filterItem}>
					<div className={s.filterField}>
						...
						<input
							onBlur={() => setActiveWith(false)}
							onFocus={() => setActiveWith(true)}
							value={searchValue}
					...
					<div className={activeWith ? s.dropList : `${s.dropList} ${s.hidden}`}>
						{toolTips.slice(0, 6).map((t, index) => (
							<div
								onClick={e => {
									setAllowedRequest([...allowedRequest, t])
									setActiveWith(false)
									setSearchValue('')
								}}
								className={s.dropList__item}
								key={index}>
				...
				<div className={s.filterItem}>
					<div className={s.filterField}>
						<img className={s.searchIcon} src={`${PF}images/icons/recipes/search.svg`} alt="" />
						<input
							onBlur={() => setActiveWithout(false)}
							onFocus={() => setActiveWithout(true)}
							className={s.filterInput}
							value={searchValueWithout}
							onChange={e => setSearchValueWithout(e.target.value)}
							type="text"
					...
					<div className={activeWithout ? s.dropList : `${s.dropList} ${s.hidden}`}>
						{toolTips.slice(0, 6).map((t, index) => (
							<div
								onClick={e => {
									setBannedRequest([...bannedRequest, t])
									setActiveWithout(false)
									setSearchValueWithout('')
								}}
								className={s.dropList__item}
								key={index}>
								<span className={s.plus}>+</span>
