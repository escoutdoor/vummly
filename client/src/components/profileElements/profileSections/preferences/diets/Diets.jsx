...
const Diets = ({ user, active, setActive, diets, setDiets, loaded }) => {
	...
	const addDiet = async diet => {
		if (user._id && diet.title.trim().length !== 0) {
			await axios
				.put(`/preferences/addDiet/${user._id}`, {
					title: diet.title,
					description: diet?.description,
				})
				.then(d => [setDiets(d.data.diets)])
		}
	}
	...
	useEffect(() => {
		setDefaultDiets(
			[...dietsMenu].filter(
				d => !diets.find(diet => diet.title === d.title && diet.description === d.description)
			)
		)
	}, [diets])
		...
		<div className={s.block}>
			<div className={s.header}>
				...
				<p className={s.description}>
					Select from the diets below and we will only show you recipes that match.
				</p>
			</div>
			<div className={s.main}>
				<div className={s.topbar}>
					...
					{diets.map(d => (
						...
						>
							<img
								className={s.bubbleImg}
								src={`${PF}images/icons/profile/preferences/${
									d.description !== ''
										? `${d.title.toLowerCase()}-${d.description
												.split(' ')
												.join('-')
												.replace(',', '')}`
										: `${d.title.toLowerCase().replace(' ', '-')}`
								}.svg`}
								alt="dietImg"
							/>
				...
				<div className={active ? `${s.menu} ${s.active}` : s.menu}>
					...
					<p className={s.description}>
						*Consult with a medical or nutrition professional before starting any dietary program to
