...
const Disliked = ({ user, active, setActive, disliked, setDisliked, loaded, ingredients }) => {
	...
	const deleteDisliked = async item => {
		if (item && user._id) {
			await axios
				.put(`/preferences/deleteDisliked/${user._id}`, {
					title: item,
				})
				.then(d => {
	...
	const addDisliked = async item => {
		if (user._id && item.trim().length !== 0) {
			await axios
				.put(`/preferences/addDisliked/${user._id}`, {
					title: item,
				})
				.then(d => [setDisliked(d.data.dislikedIngredients)])
		}
	}

	const handleAddDisliked = e => {
		e.key === 'Enter' &&
			!disliked.includes(e.target.value) &&
			defaultDisliked.find(d => d.toLowerCase() === e.target.value.toLowerCase()) &&
			addDisliked(e.target.value).then(() => {
				setNewIngredient('')
				...
				<div className={active ? `${s.menu} ${s.active}` : s.menu}>
					<div className={s.menu__content} onClick={e => e.stopPropagation()}>
						<div className={s.addDisliked}>
							<div
								onClick={() => setActiveTarget(true)}
								className={
									activeTarget ? `${s.addDisliked__target} ${s.active}` : s.addDisliked__target
								}
							>
								<span>+</span>
							...
							<div className={s.addDisliked__choose}>
								<div className={activeTarget ? `${s.inputBox} ${s.active}` : s.inputBox}>
									<input
										maxLength={30}
										ref={inputDisliked}
										onKeyDown={e => handleAddDisliked(e)}
										placeholder="add disliked foods"
										className={s.newIngredient__input}
										type="text"
										value={newIngredient}
								...
								<ul
									className={newIngredient && activeTarget ? `${s.dropdown} ${s.active}` : s.dropdown}
								>
									{defaultDisliked
										.filter(d => d.toLowerCase().includes(newIngredient.toLowerCase()))
										.slice(0, 7)
										.map((d, index) => (
											<li
												key={index}
						...
						<div className={s.bubbles}>
							{defaultDisliked.length !== 0 ? (
								defaultDisliked.slice(0, 10).map((d, index) => (
									<div
										className={s.bubble}
										key={index}
										onClick={() => {
								...
								<h2 className={s.header__title}>
									There are no more disliked ingredients, you can add yours
								</h2>
							)}
						</div>
					</div>
