...
const Allergies = ({ user, active, setActive, allergies, setAllergies, loaded }) => {
	...
	const deleteAllergy = async allergy => {
		if (allergy && user._id) {
			await axios
				.put(`/preferences/deleteAllergy/${user._id}`, {
					title: allergy,
				})
				.then(a => {
	...
	const addAllergy = async allergy => {
		if (user._id && allergy.title.trim().length !== 0) {
			await axios
				.put(`/preferences/addAllergy/${user._id}`, {
					title: allergy.title,
				})
				.then(a => [setAllergies(a.data.allergies)])
		}
	}
		...
		<div className={s.block}>
			<div className={s.header}>
				...
				<p className={s.description}>
					Select from the allergies below and we will only show you recipes that match.
				</p>
			</div>
			<div className={s.main}>
				...
				<div className={active ? `${s.menu} ${s.active}` : s.menu}>
					<div className={s.menu__content}>
						{defaultAllergies.length !== 0 ? (
							defaultAllergies.map(a => (
								<div
									className={s.bubble}
									key={a.id}
									onClick={() => {
								...
								>
									<img
										className={s.bubbleImg}
										src={`${PF}images/icons/profile/preferences/${
											a.title.toLowerCase().replace(' ', '').split('-')[0]
										}.svg`}
										alt="dietImg"
									/>
