			...
			<div className={s.menu}>
				<div className={s.bubbles}>
					{!loaded ? (
						Array(3)
							.fill(0)
							.map((a, index) => <Skeleton key={index} width={150} height={150} circle={true} />)
					) : (
						<>
							<div className={s.bubble} onClick={() => setActive(active === 'gender' ? '' : 'gender')}>
								{gender ? (
									<img className={s.icon} src={`${PF}images/icons/profile/settings/${gender}.svg`} alt="imgBubble" />
								) : (
									<img className={s.icon} src={`${PF}images/icons/profile/preferences/plus.svg`} />
								)}
								<span className={s.chosenOption}>{gender || 'gender'}</span>
							</div>
							<div className={s.bubble} onClick={() => setActive(active === 'level' ? '' : 'level')}>
								{level ? (
									<img className={s.icon} src={`${PF}images/icons/profile/settings/${level}.svg`} alt="imgBubble" />
								) : (
									<img className={s.icon} src={`${PF}images/icons/profile/preferences/plus.svg`} />
								)}
							...
							<div className={s.bubble} onClick={() => setActive(active === 'people' ? '' : 'people')}>
								{people.adults || people.kids ? (
									<img className={s.icon} src={`${PF}images/icons/profile/settings/house.svg`} alt="imgBubble" />
								) : (
									<img className={s.icon} src={`${PF}images/icons/profile/preferences/plus.svg`} />
								)}
